...
interface ExpenseFormFromMonzoProps {
    transaction: MonzoTransaction;
}

export function ExpenseFormFromMonzo({ transaction }: ExpenseFormFromMonzoProps) {
  ...
  useEffect(() => {
    const autoCategorize = async () => {
      const monzoCategory = transaction.category.replace(/_/g, ' ');
      const categoryExists = expenseCategories.some(c => c.name.toLowerCase() === monzoCategory.toLowerCase());
      if (categoryExists) {
        form.setValue("category", expenseCategories.find(c => c.name.toLowerCase() === monzoCategory.toLowerCase())!.name, { shouldValidate: true });
        return;
      }
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    addTransaction({
      id: `monzo-${transaction.id}`, // Custom ID to prevent duplicates
      type: 'expense',
      ...values,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm"><ArrowDownLeft className="mr-2 h-4 w-4"/> Import Expense</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Import Monzo Expense</DialogTitle>
        </DialogHeader>
        <Form {...form}>
